...
try
{
    ...
    using (var scope = app.Services.CreateScope())
    {
        ...
        try
        {
            var backupPerformed = await backupService.PerformAutomaticBackupAsync();
            if (backupPerformed)
            {
                Log.Information("Automatic daily backup performed on application startup");
            }
        }
        catch (Exception ex)
        {
            Log.Error(ex, "Error performing automatic backup on startup");
        }
    }
